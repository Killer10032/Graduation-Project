    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template #default="scope">
          ...
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row.aid)">
            <template #reference>
...
export default {
  ...
  methods: {
    details(row) {
      console.log(row);
      request.get("/blog/getContentByAid/" + row.aid, {
      }).then(res => {
    ...
    save() {
      ...
      if (this.form.aid) {  // 更新
        request.post("/blog/update", this.form).then(res => {
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/blog/deleteArticle/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
